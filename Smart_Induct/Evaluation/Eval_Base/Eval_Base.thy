...
fun pst_to_top_10_mods (pst:Proof.state) =
  let
    ...
    val pairs                      = try (timeout (Par_List.map ind_mods_to_pair)) lifter_modifierss |> these: (int * LiFtEr_Util.ind_mods) list;
...
let
  ...
  val top_10_triples' = try (timeout pst_to_top_10_mods) pst: triple list option;
