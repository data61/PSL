...
\<^url>\<open>https://ethz.ch/content/dam/ethz/special-interest/infk/chair-program-method/pm/documents/Verify%20This/Challenges%202019/ghc_sort.pdf\<close>\<close>
  ...
  lemma tdconc: "\<exists>ys. xs = take_decr xs @ ys" smart_induct
    apply (induction xs rule: take_decr.induct)
    apply auto
    done

  lemma ticonc: "\<exists>ys. xs = take_incr xs @ ys" smart_induct
  ...
  theorem concat_cuts: "concat (cuts xs) = xs" smart_induct(*?*)
  ...
  lemma tki: "incr (take_incr xs)" smart_induct
    apply (induction xs rule: take_incr.induct)
    apply auto
    apply (case_tac xs)
    apply auto
  ...
  lemma tkd: "decr (take_decr xs)" smart_induct
    apply (induction xs rule: take_decr.induct)
    apply auto
    apply (case_tac xs)
    apply auto
  ...
  theorem cuts_incr_decr: "\<forall>c\<in>set (cuts xs). incr c \<or> decr c" smart_induct(*?*)
    apply (induction xs rule: cuts.induct)
    apply (subst cuts.simps)
    apply (auto simp: take2_def Let_def)
    using icod by blast
  ...
  lemma maxi_imp_non_empty: "maxi xs \<Longrightarrow> []\<notin>set xs" smart_induct
    by (induction xs rule: maxi.induct) auto
        
          
  lemma tdconc': "xs\<noteq>[] \<Longrightarrow> 
    \<exists>ys. xs = take_decr xs @ ys \<and> (ys\<noteq>[] 
      \<longrightarrow> \<not>(last (take_decr xs) \<ge> hd ys))" smart_induct
    apply (induction xs rule: take_decr.induct)
    apply auto
    apply (case_tac xs) apply (auto split: if_splits)
    done
    
  lemma ticonc': "xs\<noteq>[] \<Longrightarrow> \<exists>ys. xs = take_incr xs @ ys \<and> (ys\<noteq>[] \<longrightarrow> \<not>(last (take_incr xs) < hd ys))" smart_induct
    apply (induction xs rule: take_incr.induct)
    apply auto
    apply (case_tac xs) apply (auto split: if_splits)
    done
  ...
  theorem maximal_cuts: "maxi (cuts xs)"  smart_induct
  ...
  begin
    ...
    lemma idx_spec: "fP xs \<longleftrightarrow> (\<forall>i<length xs - 1. P (xs!i) (xs!Suc i))"  smart_induct
  ...
  begin
    ...
    lemma idx_spec: "fP xs \<longleftrightarrow> (\<forall>i<length xs. P' (xs!i)) \<and> (\<forall>i<length xs - 1. P (xs!i) (xs!Suc i))" smart_induct
      apply (induction xs rule: ii_induction.induct)
      apply auto []
      apply auto []
      apply clarsimp
