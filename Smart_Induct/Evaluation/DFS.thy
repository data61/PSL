...
theory DFS
imports Main Evaluate_Smart_Induct
begin
...
lemma [simp]: "x \<notin> nodes_of g \<Longrightarrow> nexts g x = []"
  by2 (induct g) (auto simp add: nodes_of_def)
...
by2 (induct g xs ys rule: dfs2.pinduct) (force simp add: dfs2.psimps)+
...
lemma dfs_app: "dfs g (xs@ys) zs = dfs g ys (dfs g xs zs)"
  by2 (induct g xs zs rule: dfs.induct) auto

lemma "dfs2 g xs ys = dfs g xs ys" 
  by2 (induct g xs ys rule: dfs2.induct) (auto simp add: dfs_app)


subsection "Basic Properties"

lemma visit_subset_dfs: "set ys \<subseteq> set (dfs g xs ys)"
  by2 (induct g xs ys rule: dfs.induct) auto
  ...
  proof -
    {
      ...
      assume "(y,x) \<in> r\<^sup>*"
      then have "x \<in> X"
        by2 (induct) (insert assms y, auto simp add: Image_def)
    }
