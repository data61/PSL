...
lemma ins_mset:
  ...
  by2 (induct q arbitrary: t) (auto simp: union_ac link_tree_invar)
...
lemma meld_mset: "\<lbrakk>queue_invar q; queue_invar q'\<rbrakk> \<Longrightarrow> 
  ...
  queue_to_multiset q + queue_to_multiset q'"
  by2(induct q q' rule: meld.induct)
...
lemma treehead_in_multiset: 
  ...
  by2 (induct bq, simp, cases t, auto) 
...
lemma getMinTree_cons: 
  ...
  by2 (induct xs rule: getMinTree.induct) simp_all 
...
lemma remove1_mset: "t \<in> set q \<Longrightarrow> 
  ...
  by2 (induct q) (auto simp: qtm_in_set_subset)
...
next
  ...
  next
    ...
    with False have down: "getMinTree (t # bq) = getMinTree bq" 
      by2 (induct bq rule: getMinTree.induct) auto
...
proof (cases t)
  case (Node e a nat list)
  thus ?thesis by2 (induct list) simp_all
qed
