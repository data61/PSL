...
lemma ST_Suc: \<open>n \<turnstile> branch \<Longrightarrow> Suc n \<turnstile> branch\<close>
  by2 (induct n branch rule: ST.induct) (simp_all add: ST.intros)
...
lemma rev_nth_append: \<open>xs !. v = Some y \<Longrightarrow> (ys @ xs) !. v = Some y\<close>
  using rev_nth_Cons[where xs=\<open>_ @ xs\<close>] by2 (induct ys) simp_all
...
proof2 (induct xs arbitrary: v rule: rev_induct)
  case (snoc x xs)
  then show ?case
    by2 (induct v) (simp_all, metis rev_nth_snoc)
qed simp
...
lemma mapi_block_add_oob:
  ...
  using assms by2 (induct ps) simp_all
...
lemma mapi_branch_add_oob:
  ...
  unfolding mapi_branch_def using assms by2 (induct branch) simp_all
...
lemma mapi_branch_mem:
  ...
  unfolding mapi_branch_def using assms by2 (induct branch) auto
...
lemma rev_nth_mapi_branch:
  ...
  by2 (induct branch) (simp_all, metis mapi_block.simps option.inject)

lemma rev_nth_mapi_block:
  assumes \<open>ps !. v' = Some p\<close>
  shows \<open>f v' p on (mapi f ps, a)\<close>
  using assms by2 (induct ps) (simp_all, metis option.sel)
...
lemma ST_add: \<open>n \<turnstile> branch \<Longrightarrow> m + n \<turnstile> branch\<close>
  using ST_Suc by2 (induct m) auto
...
lemma finite_block_nominals: \<open>finite (block_nominals block)\<close>
  using finite_nominals by2 (induct block) auto

lemma finite_branch_nominals: \<open>finite (branch_nominals branch)\<close>
  unfolding branch_nominals_def by2 (induct branch) (auto simp: finite_block_nominals)
...
lemma sub_block_nominals: \<open>block_nominals (sub_block f block) = f ` block_nominals block\<close>
  by2 (induct block) (auto simp: sub_nominals)
...
lemma sub_branch_nominals:
  ...
  by2 (induct branch) (auto simp: sub_block_nominals)

lemma sub_list_id: \<open>sub_list id ps = ps\<close>
  using sub_id by2 (induct ps) auto

lemma sub_block_id: \<open>sub_block id block = block\<close>
  using sub_list_id by2 (induct block) auto

lemma sub_branch_id: \<open>sub_branch id branch = branch\<close>
  unfolding sub_branch_def using sub_block_id by2 (induct branch) auto
...
lemma sub_block_upd_fresh:
  ...
  using assms by2 (induct block) (auto simp add: sub_upd_fresh)
...
lemma sub_branch_upd_fresh:
  ...
  by2 (induct branch) (auto simp: sub_block_upd_fresh)
...
lemma list_down_induct [consumes 1, case_names Start Cons]:
  ...
  using assms by2 (induct ys) auto
...
proof -
  ...
  moreover have \<open>Nom i on (mapi (f l) qs, a)\<close>
    unfolding f_def using qs(2) by2 (induct qs) auto
  ultimately show ?thesis
    by blast
qed
...
lemma bridge_proper_dia:
  ...
  using assms by2 (induct p) simp_all
...
lemma descendants_initial:
  ...
  using assms by2 (induct k i branch xs rule: descendants.induct) simp_all
...
lemma descendants_bounds_fst:
  ...
  by2 (induct k i branch xs rule: descendants.induct) fast+
...
lemma descendants_bounds_snd:
  ...
  using assms
  by2 (induct k i branch xs rule: descendants.induct) (auto simp: rev_nth_Some)
...
lemma descendants_types:
  ...
  using assms by2 (induct k i branch xs arbitrary: v v' ps a) fastforce+
...
proof2 (induct n \<open>(ps, a) # branch\<close> arbitrary: ps a branch xs rule: ST.induct)
  ...
  proof (cases \<open>\<exists>a. p = Nom a\<close>)
    ...
    moreover have \<open>(\<^bold>\<not> p) on (mapi (?f w) rs, i')\<close>
      using rs(2) True by2 (induct rs) auto
    ultimately show ?thesis
...
lemma finite_nominals_set: \<open>finite S \<Longrightarrow> finite (\<Union>block \<in> S. block_nominals block)\<close>
  by2 (induct S rule: finite_induct) (simp_all add: finite_block_nominals)
...
lemma witness_used:
  ...
  using assms witness_list_used by2 (induct block) fastforce
...
lemma finite_nominals_extend:
  ...
  using assms by2 (induct n) (simp_all add: finite_block_nominals)
...
lemma consistent_extend':
  ...
  using assms by2 (induct n) (simp, metis consistent_extend finite_nominals_extend)
