...
fun postprocess_one_meth (_    :string) (0:int) (_:int) (_              )  (numbs:real option list) = numbs
 |  postprocess_one_meth (mname:string) (n:int) (t:int) (accm:real option) (numbs:real option list) =
  let
    val rank       = t - (n - 1);
    val total_occr = proc ("grep " ^ mname ^ " " ^ eval_file ^ " | wc | awk '{print $1;}'") |> #out |> Real.fromString;
...
fun postprocess_all_meths (mname::mnames:string list) (n:int) (lines:string list)=
  let
    ...
    val total_occr = proc ("grep " ^ mname ^ " " ^ path_to_Database ^ " | wc | awk '{print $1;}'") |> #out |> Int.fromString |> the |> Int.toString;
    val line = mname ^ " " ^ total_occr ^ " " ^ (space_implode " " numbs_strs);
