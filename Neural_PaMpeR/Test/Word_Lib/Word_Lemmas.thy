...
theory Word_Lemmas
  imports
    ...
    Word_Next
    Word_Enum
    "HOL-Library.Sublist" "../Assertion_Checker"
begin
...
lemma up_ucast_inj_eq:
  "len_of TYPE('a) \<le> len_of TYPE ('b) \<Longrightarrow> (ucast x = (ucast y::'b::len word)) = (x = (y::'a::len word))" assert_nth_true 45
...
lemma p_assoc_help:
  ...
  shows "p + 2^sz - 1 = p + (2^sz - 1)" assert_nth_true 4(*!*) rank_method unat_arith
  by simp

lemma word_add_increasing:
  fixes x :: "'a :: len word"
  shows "\<lbrakk> p + w \<le> x; p \<le> p + w \<rbrakk> \<Longrightarrow> p \<le> x" assert_nth_true 4 rank_method unat_arith
  by unat_arith
