...
where "msg_fresh \<sigma> m \<equiv>
         case m of Rreq hopsc _ _ _ _ oipc osnc sipc _ \<Rightarrow> osnc \<ge> 1 \<and> (sipc \<noteq> oipc \<longrightarrow>
...
lemma msg_fresh [simp]:
  "\<And>hops rreqid dip dsn dsk oip osn sip handled.
           msg_fresh \<sigma> (Rreq hops rreqid dip dsn dsk oip osn sip handled) =
...
lemma rreq_nsqn_is_fresh [simp]:
  fixes \<sigma> msg hops rreqid dip dsn dsk oip osn sip handled
  assumes "rreq_rrep_fresh (rt (\<sigma> sip)) (Rreq hops rreqid dip dsn dsk oip osn sip handled)"
      and "rreq_rrep_sn (Rreq hops rreqid dip dsn dsk oip osn sip handled)"
  shows "msg_fresh \<sigma> (Rreq hops rreqid dip dsn dsk oip osn sip handled)"
