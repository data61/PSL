...
lemma is_rreq_asm [dest!]:
  assumes "\<xi>' \<in> is_rreq \<xi>"
    shows "(\<exists>hops' dip' dsn' dsk' oip' osn' sip' handled'.
               msg \<xi> = Rreq hops' dip' dsn' dsk' oip' osn' sip' handled' \<and>
...
where
  "RREQ args \<equiv>
     \<lbrakk>\<xi>. let (hops, dip, dsn, dsk, oip, osn, sip, handled) = args \<xi> in
  ...
  "\<Gamma>\<^sub>A\<^sub>O\<^sub>D\<^sub>V PAodv = labelled PAodv (
     ...
     (    \<langle>is_newpkt\<rangle> NEWPKT(\<lambda>\<xi>. (data \<xi>, ip \<xi>))
       ...
       \<oplus> \<langle>is_rreq\<rangle>
            ...
            RREQ(\<lambda>\<xi>. (hops \<xi>, dip \<xi>, dsn \<xi>, dsk \<xi>, oip \<xi>, osn \<xi>, sip \<xi>, handled \<xi>))
     ...
     \<oplus> \<langle>\<lambda>\<xi>. { \<xi>\<lparr> dip := dip \<rparr>
         ...
         broadcast(\<lambda>\<xi>. rreq(0, dip \<xi>, sqn (rt \<xi>) (dip \<xi>), sqnf (rt \<xi>) (dip \<xi>), ip \<xi>, sn \<xi>,
                            ip \<xi>, False)). AODV())"
         ...
         (
           \<langle>\<xi>. dip \<xi> = ip \<xi>\<rangle>
             ...
             unicast(\<lambda>\<xi>. the (nhop (rt \<xi>) (oip \<xi>)), \<lambda>\<xi>. rrep(0, dip \<xi>, sn \<xi>, oip \<xi>, ip \<xi>)).
               broadcast(\<lambda>\<xi>. rreq(hops \<xi> + 1, dip \<xi>, dsn \<xi>, dsk \<xi>, oip \<xi>, osn \<xi>, ip \<xi>, True)).
               AODV()
             \<triangleright>
