...
lemma nsqn\<^sub>r_zero [simp]:
  "\<And>dsn dsk flag hops nhip. nsqn\<^sub>r (0, dsk, flag, hops, nhip) = 0"
  unfolding nsqn\<^sub>r_def by clarsimp

lemma nsqn\<^sub>r_val [simp]:
  "\<And>dsn dsk hops nhip. nsqn\<^sub>r (dsn, dsk, val, hops, nhip) = dsn"
  unfolding nsqn\<^sub>r_def by clarsimp

lemma nsqn\<^sub>r_inv [simp]:
  "\<And>dsn dsk hops nhip. nsqn\<^sub>r (dsn, dsk, inv, hops, nhip) = dsn - 1"
  unfolding nsqn\<^sub>r_def by clarsimp

lemma nsqn\<^sub>r_lte_dsn [simp]:
  "\<And>dsn dsk flag hops nhip. nsqn\<^sub>r (dsn, dsk, flag, hops, nhip) \<le> dsn"
  unfolding nsqn\<^sub>r_def by clarsimp
...
lemma nsqnr_r_flag_pred [simp, intro]:
  ...
  assumes "P (nsqn\<^sub>r (dsn, dsk, val, hops, nhip))"
    ...
    shows "P (nsqn\<^sub>r (dsn, dsk, flag, hops, nhip))"
...
theorem rt_fresher_update [simp]:
  ...
  next
    ...
    from \<open>dip\<in>kD(rt)\<close> obtain dsn\<^sub>n dsk\<^sub>n f\<^sub>n hops\<^sub>n nhip\<^sub>n
      where rtn [simp]: "the (rt dip) = (dsn\<^sub>n, dsk\<^sub>n, f\<^sub>n, hops\<^sub>n, nhip\<^sub>n)"
    ...
    from \<open>update_arg_wf r\<close> have "(dsn\<^sub>n, dsk\<^sub>n, f\<^sub>n, hops\<^sub>n, nhip\<^sub>n)
                                  \<sqsubseteq> the ((update rt dip r) dip)"
      proof (rule wf_r_cases)
        fix nhip pre
        from \<open>hops\<^sub>n \<ge> 1\<close> have "\<And>pre'. (dsn\<^sub>n, dsk\<^sub>n, f\<^sub>n, hops\<^sub>n, nhip\<^sub>n)
                                        \<sqsubseteq> (dsn\<^sub>n, unk, val, Suc 0, nhip)"
          unfolding fresher_def sqn_def by (cases f\<^sub>n) auto
        thus "(dsn\<^sub>n, dsk\<^sub>n, f\<^sub>n, hops\<^sub>n, nhip\<^sub>n)
      ...
      next
        ...
        assume "0 < dsn"
        show "(dsn\<^sub>n, dsk\<^sub>n, f\<^sub>n, hops\<^sub>n, nhip\<^sub>n)
        ...
        next
          assume "dsn\<^sub>n = dsn"
             and "hops < hops\<^sub>n"
            thus "(dsn, dsk\<^sub>n, f\<^sub>n, hops\<^sub>n, nhip\<^sub>n)
                   \<sqsubseteq> (dsn, kno, val, hops, nhip)"
              unfolding fresher_def nsqn\<^sub>r_def by simp
        next
          assume "dsn\<^sub>n = dsn"
            with \<open>0 < dsn\<close>
            show "(dsn, dsk\<^sub>n, inv, hops\<^sub>n, nhip\<^sub>n)
                   \<sqsubseteq> (dsn, kno, val, hops, nhip)"
              unfolding fresher_def by simp
        qed
      qed
