...
Turn the Muddy Chilkdren / clock trie into a DFA and reduce it under
bisimulation.

Run the DFS on the example, copy the result into MC_clock.hs:

-- Apparently the show instance is deeply nested
ghci -fcontext-stack=50 MuddyChildren.hs
ghci> mc_ClockDFS Child0

Initial states for Child0:

:t [ (s, ClockView.clock_simInit envInit envObs Child0 (envObs Child0 s)) | s <- envInit ]

Initial transitions are labelled with the scenario: (isMuddy0, isMuddy1, isMuddy2)
Later transitions with the children's actions.
...
--      numStates dfa >>= \n -> putStrLn $ " DFA: " ++ show n
     ...
     writeDotToFile dfa "mc_clock.dot" llim
     return ()
