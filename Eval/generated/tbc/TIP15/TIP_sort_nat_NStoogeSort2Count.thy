(* This Isabelle theory is produced using the TIP tool offered at the following website: 
  ...
  theory TIP_sort_nat_NStoogeSort2Count
  imports Main TBC.TBC
begin
...
(*fun did not finish the proof*)
function twoThirds :: "Nat => Nat" where
  "twoThirds y =
   (if (y = (S (S Z))) then S Z else
      (if (y = (S Z)) then S Z else
         (case y of
            Z => Z
            | S z => plus (S (S Z)) (twoThirds (minus y (S (S (S Z))))))))"
  by pat_completeness auto
...
function nstooge2sort2 :: "Nat list => Nat list"
  and nstoogesort2 :: "Nat list => Nat list"
  and nstooge2sort1 :: "Nat list => Nat list" where
  "nstooge2sort2 y =
...
evaluate_tbc property0 :
  "((count y (nstoogesort2 xs)) = (count y xs))"
end
