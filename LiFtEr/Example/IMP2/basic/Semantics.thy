...
theory Semantics
imports Syntax "HOL-Eisbach.Eisbach_Tools"  "../../Concrete_Semantics/Induction_Demo"
begin
...
fun small_step :: "program \<Rightarrow> com \<times> state \<Rightarrow> (com \<times> state) option" where
  "small_step \<pi> (AssignIdx x i a,s) = Some (SKIP, s(x := (s x)(aval i s := aval a s)))"
| "small_step \<pi> (ArrayCpy x y,s) = Some (SKIP, s(x := s y))"  
| "small_step \<pi> (ArrayClear x,s) = Some (SKIP, s(x := (\<lambda>_. 0)))"
...
open LiFtEr;
infix And Imply Is_An_Arg_Of Is_Rule_Of Is_Nth_Ind Is_In_Trm_Loc Is_In_Trm_Str Trm_Occ_Is_Of_Trm;
   ...
   Some_Trm_Occ_Of (Trm_Occ 1, Trm 1,
    ...
    (All_Ind (Trm 2,
     (Some_Trm_Occ_Of (Trm_Occ 2, Trm 2,
       ...
       And
       (All_Trm_Occ (Trm_Occ 3,
          (Not (Is_Atom (Trm_Occ 2)))
          And
          (Is_Free (Trm_Occ 3)
          And
           (Trm_Occ 3 Is_In_Trm_Loc Trm_Occ 2))
         Imply
         (Some_Arb (Trm 3,
           Trm_Occ 3 Trm_Occ_Is_Of_Trm Trm 3)))))))))));

val difficult_prf =
Ind_Mods
 {ons   = [Ind_On  (Print "\<pi>"), Ind_On (Print "(c,s)"), Ind_On (Print "Some (c',s')")],
  arbs  = [Ind_Arb (Print "c"), Ind_Arb (Print "s"), Ind_Arb (Print "c'"), Ind_Arb (Print "s'")],
  rules = [Ind_Rule "small_steps.induct"]
  }: ind_mods;

end;
\<close>
setup\<open> Apply_LiFtEr.update_assert  "difficult_assrt" difficult_assrt \<close>
setup\<open> Apply_LiFtEr.update_ind_mod "difficult_prf"   difficult_prf \<close>

lemma smalls_seq: "\<lbrakk>small_steps \<pi> (c,s) (Some (c',s'))\<rbrakk> \<Longrightarrow> small_steps \<pi> (c;;cx,s) (Some (c';;cx,s'))"
  assert_LiFtEr difficult_assrt [on ["\<pi>", "(c,s)", "Some (c',s')"], arb ["c", "s", "c'", "s'"], rule ["small_steps.induct"]]
