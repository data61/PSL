...
proof -
  have "addO n (exp\<omega> m) = addO n' (exp\<omega> m') \<Longrightarrow> n = n'"
    by2 (induct m arbitrary: m'; case_tac m';
...
lemma C2O_inj:
  ...
  by2 (induct n arbitrary: m rule: C2O.induct; case_tac m rule: C2O.cases)
...
lemma C2O_app:
  ...
  by2 (induct xs arbitrary: ys) auto

subsection \<open>Evaluation\<close>

lemma evalC_def':
  "evalC b n = evalO b (C2O n)"
  by2 (induct n rule: C2O.induct) auto
...
lemma C2O_cons:
  ...
  by2 (induct n arbitrary: ns rule: funC.induct)
...
proof -
  have "\<forall>n. C2O n = m \<longrightarrow> P n" for m
    by2 (induct m; intro allI; case_tac n rule: funC.cases)
...
proof -
  have "goodsteinC_dom (c, n)" for c n
    by2 (induct n arbitrary: c rule: C_Ord_induct) (auto intro: goodsteinC.domintros)
  then show ?thesis by simp
qed

lemma goodsteinC_def':
  "goodsteinC c n = goodsteinO c (C2O n)"
  by2 (induct c n rule: goodsteinC.induct) (simp_all add: C2O_cons del: C2O.simps(2))
...
proof -
  have "stepC_dom (c, n)" for c n
    by2 (induct n arbitrary: c rule: C_Ord_induct) (auto intro: stepC.domintros)
  then show ?thesis by simp
qed
...
lemma stepC_def':
  ...
  by2 (induct c n rule: stepC.induct) (simp_all add: C2O_cons del: C2O.simps(2))
...
lemma evalC_funC [simp]:
  ...
  by2 (induct n rule: funC.induct) simp_all

lemma stepC_app [simp]:
  "n \<noteq> C [] \<Longrightarrow> stepC c (C (unC n @ ns)) = C (unC (stepC c n) @ ns)"
  by2 (induct n arbitrary: ns rule: stepC.induct) simp_all
...
lemma stepC_dec:
  ...
  by2 (induct c n rule: stepC.induct) simp_all
...
proof2 (induct c n rule: stepC.induct)
  ...
  have "evalC (c+3) (C (funC (C (n # ns)) (Suc (Suc c)))) \<le>
    ...
    by2 (induct n rule: funC.induct) (simp_all add: distrib_right)
...
next
  have [simp]: "Suc x \<le> Suc (Suc b)^x" for x by2 (induct x) auto
  case (Suc n)
...
lemma hbase_mono:
  ...
  by2 (induct n rule: hbase.induct) (auto 0 3 intro: hbase.intros hbase_evalC_mono)
...
proof (relation "measure (\<lambda>(c, n). goodsteinC c (N2H (c+1) n) - c)", goal_cases _ 1)
  ...
  have *: "goodsteinC c n \<ge> c" for c n
    by2 (induct c n rule: goodsteinC.induct) simp_all
...
lemma goodstein_def':
  ...
  by2 (induct c n rule: goodstein.induct) (simp_all add: goodstein_aux eval_nat_numeral)
...
lemma mul\<^sub>O:
  ...
  by2 (induct m) (simp_all add: add\<^sub>O)
...
proof -
  have [simp]: "\<langle>(S ^^ i) Z\<rangle>\<^sub>O z s l = \<langle>i\<rangle>\<^sub>N s z" for i z s l by2 (induct i) simp_all
...
lemma exp\<omega>\<^sub>O:
  ...
  by2 (induct n) (simp_all add: mul\<^sub>O \<omega>\<^sub>O)
...
lemma goodstein\<^sub>O:
  ...
  by2 (induct n arbitrary: c) simp_all
...
lemma freeOrd:
  ...
  by2 (induct n) (simp_all add: assms)
