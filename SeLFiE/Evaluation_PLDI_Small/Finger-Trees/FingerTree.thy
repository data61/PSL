...
lemma gmft_correct: "is_measured_ftree t 
  ...
  apply (induct t) by( auto simp add: ft_invar_def gmd_correct gmn_correct add.assoc)
...
lemma toList_empty: "toList t = [] \<longleftrightarrow> t = Empty"
  apply (induct t) by(auto simp add: nodeToList_empty digitToList_empty)
...
lemma nlcons_invlevel: "\<lbrakk>is_leveln_ftree n t; is_leveln_node n nd\<rbrakk> 
  ...
  apply (induct t arbitrary: n nd rule: nlcons.induct) 
...
lemma nlcons_invmeas: "\<lbrakk>is_measured_ftree t; is_measured_node nd\<rbrakk> 
  ...
  apply (induct t arbitrary: nd rule: nlcons.induct) 
  by (auto simp add: deep_def node3_def)
...
lemma lconsNlist_inv:
  ...
  apply (insert assms) apply(induct xs) by(auto simp add: nlcons_invlevel nlcons_invmeas)
...
lemma rconsNlist_inv:
  ...
  apply (insert assms) apply(induct xs arbitrary: t) by(auto simp add: nrcons_invlevel nrcons_invmeas)
...
lemma nodes_inv:
  ...
  apply (insert assms) proof(induct ts rule: nodes.induct)
  case (1 a b)
...
apply (insert assms) proof(induct t1 xs t2 arbitrary: n rule: app3.induct)
  case (1 xs t n)
...
lemma nodes_list: "length xs \<ge> 2 \<Longrightarrow> nlistToList (nodes xs) = nlistToList xs"
  apply (induct xs rule: nodes.induct) by(auto simp add: node2_def node3_def)

lemma nlistToList_app: 
  "nlistToList (xs@ys) = (nlistToList xs) @ (nlistToList ys)"
  apply (induct xs arbitrary: ys) by(simp_all)

lemma nlistListLCons: "toList (lconsNlist xs t) = (nlistToList xs) @ (toList t)"
  apply (induct xs) by(auto simp add: nlcons_list)
 
lemma nlistListRCons: "toList (rconsNlist t xs) = (toList t) @ (nlistToList xs)"
  apply (induct xs arbitrary: t) by(auto simp add: nrcons_list)
...
lemma nlistToTree_inv: 
  ...
  apply (unfold nlistToTree_def) apply(induct nl) apply(auto simp add: nlcons_invmeas)
  apply(induct nl) by(auto simp add: nlcons_invlevel)
...
lemma deepL_list:
  ...
  apply (insert assms) proof(induct "pr" m sf rule: deepL.induct)
  case (1 m sf)
  thus ?case
...
lemma deepR_list:
  ...
  apply (insert assms) proof(induct "pr" m sf rule: deepR.induct)
  case (1 "pr" m)
  thus ?case
  proof (auto split: viewnres_split simp add: deep_def)
...
lemma gmnl_correct:
  ...
  apply (insert assms) apply(induct xs) by(auto simp add: add.assoc gmn_correct)
...
lemma foldl_node_correct: 
  ...
  apply (induct nd arbitrary: "\<sigma>") by(auto simp add: nodeToList_def)

lemma foldl_digit_correct:
  "foldl_digit f \<sigma> d = List.foldl f \<sigma> (digitToList d)"
  apply (induct d arbitrary: "\<sigma>") by(auto 
...
lemma foldl_correct:
  ...
  apply (induct t arbitrary: "\<sigma>") by(auto 
...
lemma count_node_correct:
  ...
  apply (induct n) by(auto simp add: nodeToList_def count_node_def)
