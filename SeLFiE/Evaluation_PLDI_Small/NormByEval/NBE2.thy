...
*)
(*<*)
theory NBE2 imports Main "Eval_Base.Eval_Base" begin
...
lemma depth_At_foldl:
 "depth_At(s \<bullet>\<bullet> ts) = depth_At s + size ts"
  apply (induct ts arbitrary: s) by simp_all
...
apply (induct arbitrary: s s' rule:list_induct2) by simp_all
...
apply (induct ts arbitrary:t) by auto
...
apply(induct ts arbitrary: s) by auto
...
apply(induct arbitrary:k pred:pure) by simp_all
...
apply(induct k v rule: lift_ML.induct) by (simp_all add:list_eq_iff_nth_eq)
...
apply (induct \<sigma> v arbitrary: k \<sigma>' rule: subst_ML.induct) by auto
...
apply (induct k v arbitrary: n rule: lift_ML.induct) by simp_all
...
apply(induct \<sigma> v arbitrary: k rule: subst_ML.induct) by(auto simp: closed_ML_Suc)
...
apply(induct \<sigma> v arbitrary: k l rule: subst_ML.induct) by(auto simp: closed_ML_Suc)
...
apply(induct pred:pure) by simp_all
...
| ctxt_apply: "t \<Rightarrow> t'   \<Longrightarrow> apply s t \<Rightarrow> apply s t'"
...
using assms apply (induct) by simp_all
...
apply(induct \<sigma>' v rule:subst_ml.induct) by(simp_all add:list_eq_iff_nth_eq)
...
apply(induct t pred:pure) by (simp_all add:comp_open_def)

lemma fv_ML_comp_pat: "pattern t \<Longrightarrow> fv\<^sub>M\<^sub>L(comp_pat t) = fv t"
apply(induct t pred:pattern) by (simp_all add:comp_open_def)
...
apply (induct pred:pure) by auto
...
lemma dterm_ML_comp_patD:
  ...
  apply(induct pred:pattern) by simp_all
...
apply(induct rule: Red_ml_Red_ml_list.inducts) by simp_all
...
apply(induct v arbitrary: k rule:C_normal_ML.induct) by (auto simp:no_match_ML_lift_ML)
...
apply (induct pred:pure) by (auto simp:comp_open_def)
...
apply (induct ts rule:rev_induct) by auto
...
lemma C_normal_ML_compile:
  ...
  apply(induct t arbitrary: \<sigma>) by(simp_all add: C_normal_ML_lift_ML)
...
apply(induct pred:pattern) by auto
