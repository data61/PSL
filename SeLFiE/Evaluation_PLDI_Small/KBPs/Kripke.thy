...
lemma kripke_tc_rels_worlds[dest]:
  ...
  assumes M: "kripke M"
  shows "w \<in> worlds M \<and> w' \<in> worlds M"
  using assms apply2 (induct rule: trancl_induct) by auto
...
lemma kripke_rels_trc_worlds:
  ...
  shows "w' \<in> W"
  using assms apply2 (induct rule: rtrancl_induct) by auto
...
proof -
  { fix w' assume ww': "(w, w') \<in> (\<Union>x\<in>set as. relations M x)\<^sup>+"
    from ww' S5n E p w have "M, w' \<Turnstile> Kand \<phi> \<psi>"
      apply2 ( induct rule: trancl_induct)
      by( simp_all, blast+) }
  thus ?thesis by simp
qed
...
using R
proof2(induct rule: trancl_induct)
...
proof -
  { fix x assume "(t, x) \<in> (\<Union>x. relations M x)\<^sup>*"
    ...
    proof2(induct rule: rtrancl_induct)
      case (step y z)
...
(*<*)
  using assms
  apply - apply2( induct rule: trancl_induct[OF uv'])
  by( auto dest: sim_fD intro: trancl_into_trancl )
(*>*)
text\<open>
...
lemma sim_f_trc:
  ...
  using assms
  apply - apply2( induct rule: rtrancl_induct[OF uv'])
  by( auto dest: sim_fD intro: rtrancl_into_rtrancl )
