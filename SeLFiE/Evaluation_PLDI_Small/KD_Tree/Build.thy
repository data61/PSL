...
lemma widest_spread_calc_spread:
  ...
  apply (induction ks ps rule: widest_spread.induct) by (auto simp: Let_def split: prod.splits if_splits)
...
proof -
  ...
  also have "... = length (filter (\<lambda>v. v \<le> sort ?ps ! ?n) ?ps)"
    apply (induction ps) by (auto, metis comp_apply)
  also have "... = ?n + 1"
...
proof (induction ps rule: length_induct)
  ...
  consider (A) "xs = []" | (B) "\<exists>x. xs = [x]" | (C) "\<exists>x y zs. xs = x # y # zs"
    apply (induction xs rule: induct_list012) by auto
  then show ?case
  proof cases
    case C
