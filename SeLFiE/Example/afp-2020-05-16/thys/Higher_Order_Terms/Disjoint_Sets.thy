...
lemma disjnt_insert: "x \<notin> N \<Longrightarrow> disjnt M N \<Longrightarrow> disjnt (insert x M) N"
unfolding disjnt_def by simp

lemma disjnt_ge_max:
  assumes "finite Y" "\<And>x. x \<in> X \<Longrightarrow> x > Max Y"
  shows "disjnt X Y"
unfolding disjnt_def
by (meson Int_emptyI Max.coboundedI assms(1) assms(2) leD)
