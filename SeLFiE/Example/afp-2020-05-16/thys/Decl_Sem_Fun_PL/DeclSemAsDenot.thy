...
fun E :: "exp \<Rightarrow> env \<Rightarrow> val set" where
  ...
  Elam: "E (ELam x e) \<rho> = { v. \<exists> f. v = VFun f \<and> (\<forall> v1 v2. (v1, v2) \<in> set f 
    \<longrightarrow> v2 \<in> E e ((x,v1)#\<rho>)) }" |
  Eapp: "E (EApp e1 e2) \<rho> = { v3. \<exists> f v2 v2' v3'. 
      VFun f \<in> E e1 \<rho> \<and> v2 \<in> E e2 \<rho> \<and> (v2', v3') \<in> set f \<and> v2' \<sqsubseteq> v2 \<and> v3 \<sqsubseteq> v3' }" |
